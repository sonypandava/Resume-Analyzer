...
def fetch_yt_video(link):
    try:
        ...
        video_title = info_dict.get('title', 'Video Title Not Available')
        return video_title, video_url
    except Exception as e:
        logging.error(f"Error fetching YouTube video: {e}")
        return None, None

# Function to randomly select and display a video title
def display_random_title(video_list):
    random_video = random.choice(video_list)
    st.subheader("✅ **Video Title:** " + random_video)

# Function to randomly select and display a video link
def display_random_video(video_list):
    random_video = random.choice(video_list)
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                display_random_title(resume_videos)
                display_random_video(resume_videos)

                ## Interview Preparation Video
                st.header("**Bonus Video for Interview👨‍💼 Tips💡**")

                display_random_title(interview_videos)
                display_random_video(interview_videos)
        ...
        if st.button('Login'):
            if ad_user == 'saikiran' and ad_password == 'sai12345':
                st.success("Welcome sai")
                # Display Data
